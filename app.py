...
@app.get("/ui/api/trakt/status")
@auth.require_auth
def ui_api_trakt_status():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    import trakt
    return jsonify(**trakt.status(rec))


@app.post("/ui/api/trakt/auth/start")
@_csrf.exempt
@auth.require_auth
def ui_api_trakt_auth_start():
    import trakt
    if not trakt.is_configured():
        return jsonify(error="Trakt is not configured (missing client id/secret in Settings)"), 400
    data = trakt.request_device_code()
    if not data:
        return jsonify(error="Could not start Trakt device authorization"), 502
    from flask import session as _session
    _session["trakt_device_code"] = data.get("device_code")
    return jsonify(
...
@app.get("/ui/api/trakt/auth/poll")
@auth.require_auth
def ui_api_trakt_auth_poll():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    from flask import session as _session
    device_code = _session.get("trakt_device_code")
    if not device_code:
        return jsonify(status="error", error="no pending authorization")
    import trakt
    result = trakt.poll_device_token(device_code)
    if result["status"] == "success":
        _session.pop("trakt_device_code", None)
        username = None
        try:
            import requests as _rq
            r = _rq.get(
                "https://api.trakt.tv/users/me",
                headers=trakt._headers(result["access_token"]), timeout=10,
            )
            if r.status_code == 200:
        ...
        db.update_user(
            rec["id"],
            trakt_access_token=result["access_token"],
            trakt_refresh_token=result["refresh_token"],
            trakt_token_expires=result["expires_at"],
...
@app.post("/ui/api/trakt/auth/revoke")
@_csrf.exempt
@auth.require_auth
def ui_api_trakt_auth_revoke():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    import trakt
...
@app.post("/ui/api/trakt/sync")
@_csrf.exempt
@auth.require_auth
def ui_api_trakt_sync():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    import trakt
    added = trakt.sync_watchlist_auto_request(rec)
    return jsonify(ok=True, added=added)


@app.post("/ui/api/trakt/sync-watched")
@_csrf.exempt
@auth.require_auth
def ui_api_trakt_sync_watched():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    import trakt
    watched = trakt.sync_watched(rec)
    return jsonify(ok=True, watched=watched)


@app.get("/ui/api/trakt/watched")
@auth.require_auth
def ui_api_trakt_watched():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    rows = db.get_trakt_watched_movies(rec["id"])
    return jsonify(imdb_ids=[r["imdb_id"] for r in rows if r.get("imdb_id")])


@app.get("/ui/api/trakt/watched-episodes")
@auth.require_auth
def ui_api_trakt_watched_episodes():
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    return jsonify(shows=db.get_trakt_watched_episodes(rec["id"]))


@app.post("/ui/api/trakt/scrobble")
@_csrf.exempt
@auth.require_auth
def ui_api_trakt_scrobble():
    """Forward webplayer playback progress to Trakt's scrobble API (best-effort)."""
    rec = auth.current_user_record()
    if not rec:
        return jsonify(error="not authenticated"), 401
    import trakt
    token = trakt._valid_access_token(rec)
    if not token:
        return jsonify(ok=False, error="Trakt not connected")
    p = request.get_json(silent=True) or {}
    imdb_id = p.get("imdb_id")
    progress = p.get("progress")
    action = p.get("action") if p.get("action") in ("start", "pause", "stop") else None
    if not imdb_id or progress is None or not action:
        return jsonify(error="imdb_id, progress and a valid action are required"), 400
    body: dict = {"progress": float(progress)}
    if p.get("season") is not None and p.get("episode") is not None:
        body["show"] = {"ids": {"imdb": imdb_id}}
        body["episode"] = {"season": p["season"], "number": p["episode"]}
    else:
        body["movie"] = {"ids": {"imdb": imdb_id}}
    try:
        import requests as _rq
        _rq.post(f"https://api.trakt.tv/scrobble/{action}",
                 json=body, headers=trakt._headers(token), timeout=10)
    except Exception as exc:
        log.debug("Trakt scrobble failed: %s", exc)
    return jsonify(ok=True)
