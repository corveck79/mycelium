...
CMD ["sh", "-c", "[ \"${SPORE_NFS_ENABLED:-true}\" = true ] && LISTEN_ADDR=:2049 spore-nfs & [ \"${SPORE_SMB_ENABLED:-true}\" = true ] && LISTEN_ADDR=0.0.0.0:445 spore-smb & exec gunicorn --bind ${LISTEN_HOST}:${LISTEN_PORT} --workers 1 --threads 16 --access-logfile - app:app"]
